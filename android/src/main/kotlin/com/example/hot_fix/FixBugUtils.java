...
public class FixBugUtils {
  ...
  private final File mDexDir;//应用可以访问的dex目录

  public FixBugUtils(Context context) {
    this.mContext = context;
    //获取到应用可以访问的dex目录
    this.mDexDir = context.getDir("odex", Context.MODE_PRIVATE);
    Log.i(TAG, mDexDir.getAbsolutePath());
  }
  public File getPath()  {
    return mDexDir;
  }
  ...
  private Object getElementsByClassLoader(ClassLoader applicationClassLoader) throws NoSuchFieldException, IllegalAccessException {
    //1.先获取ClassLoader里面的pathList
    @SuppressLint("DiscouragedPrivateApi") Field pathListFiled = BaseDexClassLoader.class.getDeclaredField("pathList");
  ...
  public void fixDex(String fixDexPath) throws Exception {
    ...
    FileUtils.copyFile(srcFile, destFile);// copy to patch's directory
//        FileUtil.deleteFile(srcFile);//copy完成后删除
  ...
  private void fixDexFiles(List<File> fixDexFiles) throws Exception {
    ...
    for (File fixDexFile : fixDexFiles) {
      Log.i(TAG, "fix :" + fixDexFile.getPath());
      //dexPath 加载的dex路径
